...
public class AppointmentController {
	...
	// Add Appointment
		@RequestMapping(value = "/appointment", method = RequestMethod.POST, produces = { MediaType.APPLICATION_JSON_VALUE,
				MediaType.APPLICATION_XML_VALUE })
		@ResponseBody
		public ResponseEntity<AppointmentOrder> addMyAppointment(SecurityContextHolderAwareRequestWrapper request, @RequestBody AppointmentOrder data) {
			HttpStatus httpStatus = null;
			String username = request.getUserPrincipal().getName();
			AppointmentOrder appointmentOrder = new AppointmentOrder();
			try {
				appointmentOrder = appoitmentService.addAdAppointmentOrder(data, username);
				...
				// TODO: handle exception
				httpStatus = HttpStatus.INTERNAL_SERVER_ERROR;
				System.out.print("Loi add appointment " + e.toString());
			}
			
			return new ResponseEntity<AppointmentOrder>(appointmentOrder, httpStatus);
		}
